...
const CoversationPartner = ({ onUserSelect }) => {
  // Add onUserSelect prop here
  const partners = [
    {
      id: 1,
      name: "Rex Allen",
      ...
      <div className="bg-whiteColor dark:bg-whiteColor-dark border border-borderColor dark:border-borderColor-dark rounded-lg2 max-h-150 overflow-auto">
        ...
        <ul>
          {partners?.map(({ name, image, lastMessage, lastSeen }, idx) => (
            <li
              key={idx}
              className="border-t border-borderColor dark:border-borderColor-dark cursor-pointer hover:bg-gray-50"
              onClick={() => onUserSelect({ name, image })}
            >
