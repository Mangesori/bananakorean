  ...
  function playVideos(e) {
    ...
    if (url.indexOf('youtube') !== -1 || url.indexOf('youtu') !== -1) {
      ...
      for (i = 0; i < ytUrl.length; ++i) {
        ...
        document.getElementById('lvideo-wrap').innerHTML =
          `${startModal} <iframe width="560" height="315" title="YouTube Video" src='https://www.youtube.com/embed/${r[1]}?rel=0&autoplay=0&mute=0&loop=1&playlist=${r[1]}' frameborder="0" allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>${finishModal}`;
      }
      ...
      if (match) {
        document.getElementById('lvideo-wrap').innerHTML =
          `${startModal}<iframe title="Vimeo" src="https://player.vimeo.com/video/${match[2]}?autoplay=1&loop=1" width="640" height="360" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>${finishModal}`;
      } else {
        alert('Not a Vimeo!  URL example:\n https://vimeo.com/120206922');
      }
    } else if (url.indexOf('mp4') !== -1 || url.indexOf('m4v') !== -1) {
      document.getElementById('lvideo-wrap').innerHTML =
        `${startModal}<video controls loop playsinline autoplay><source src='${this.dataset.url}' type="video/mp4"></video>${finishModal}`;
    } else {
