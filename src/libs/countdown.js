const countDown = () => {
  ...
  if (coundownContainers?.length) {
    ...
    coundownContainers.forEach(coundownContainer => {
      ...
      const targetDateArray = coundownContainer.getAttribute('data-countdown').split('/');
      ...
      countdownInterval = setInterval(() => updateCountdown(targetDate, countDownFields), 1000);
    });
